...
namespace adaptive_betweenness_centrality {
	...
	void bfs(g_t& G, int s, std::vector<int>& dists, std::vector<int>& nums) {
		...
		while (qh != qt) {
			...
			rep(i, G[u].size()) {
				...
				if (dists[v] == -1) {
					dists[v] = dists[u] + 1;
					// adding this one next line to make it k-centrality
					//if (dists[v] <= k) // added this line only
					    q[qt++] = v;
				}
			}
		}
	...
	void load_graph(const std::string& fn, std::vector<std::vector<int>> & G) {
	...
	void load_graph_from_Graph(const std::string& fn, g_t& G) {
		bool weighted = false;
		std::ifstream ifs(fn.c_str());
		for (std::string line; getline(ifs, line); ) {
