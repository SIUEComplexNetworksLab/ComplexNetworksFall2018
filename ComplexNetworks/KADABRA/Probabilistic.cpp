...
#include <math.h>
#include <limits.h>
//#include <iomanip>  // currently unused and causes ctime errors with undelcared variables -DB
//#include <time.h> // testing use without time.h becuase it uses windows.h which cant be used on Linux
#include <cfloat>
#include <omp.h>
#include <algorithm>
#include "utilities.h"
...
Probabilistic::Probabilistic(const GraphOrig & filename, const bool directed, const double verb) : Graph3(filename), verbose(verb) {
...
// Creates the graph for running the approximation algorithm.
// For more information see the graph class.
Probabilistic::Probabilistic( const std::string &filename, const bool directed, const double verb ): Graph3( filename, directed ), verbose(verb) {
    approx = (double *) calloc( get_nn(), sizeof(double) );
    delta_l_guess = (double *) calloc( get_nn(), sizeof(double) );
    delta_u_guess = (double *) calloc( get_nn(), sizeof(double) );
    time_bfs = (double *) calloc( omp_get_max_threads(), sizeof(double) );
