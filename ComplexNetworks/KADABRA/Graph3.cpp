...
// shuffle between graph formats


Graph3::Graph3(const GraphOrig & myGraph)
{
	directed = false;
	vector< vector< int > > pre_adj(0);
	vector< vector< int > > pre_inc(0);
	uint32_t u = 0, v = 0;
	string line;
	...
	*/
	...
	pre_adj = myGraph.nodes;
	if (directed) 
	{
		// we only need to do undirected... our .graph  files work the same as undirected here, have edges going both ways
	}
...
Graph3::Graph3( const uint32_t nodes_num, const uint32_t *maxdegs ) {
...
Graph3::Graph3( const string &graph_path, const bool directed ) : directed(directed)
{
...
void Graph3::compute_pivot( uint32_t *pivots ) {
...
uint32_t Graph3::compute_ecc_in_scc(const uint32_t start, uint32_t *q, int32_t *dist, const bool backward) {
...
uint32_t Graph3::estimate_diameter() {
    compute_cc();
    compute_cc_adj();
...
void Graph3::add_edge( const uint32_t u, const uint32_t v ) {
    adj[u][degrees[u]++] = v;
}
...
void Graph3::remove_some_edges( const uint32_t *vertices, const uint32_t length) {
