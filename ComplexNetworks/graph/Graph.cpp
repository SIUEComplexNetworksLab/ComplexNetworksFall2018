...
/** CONSTRUCTORS **/
	...
	Graph::Graph(const GraphOrig & myGraph) 
	{
		n=myGraph.nodes.size();
		m = 0;
		for (size_t i = 0; i < myGraph.nodes.size(); i++)
		{
			m += myGraph.nodes[i].size();
		}
		m /= 2;
		...
		edgesIndexed = false;
		id = getNextGraphId();
		name =  myGraph.orig_file;
		for (size_t i = 0; i < n; i++)
		{
			if (myGraph.nodes[i].size() == 0) {
				exists.push_back(false);
			}
			else {
				exists.push_back(true);
			}
			outDeg.push_back(myGraph.nodes[i].size());
			vector<NetworKit::count> newVect;
			for (size_t j = 0; j < myGraph.nodes[i].size(); j++)
			{
				newVect.push_back(myGraph.nodes[i][j]);

			}
			outEdges.push_back(newVect);
		}
	}
